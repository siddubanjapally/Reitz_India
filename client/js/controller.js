...
(function() {
  (angular.module('reitz')).controller('chartCtrl', function($scope, $location, $rootScope, $modal, $routeParams, $timeout, ngTableParams, $filter, $http, projectservice, chartService, ReitzResources) {
    var calRpm, calShaftBrg, calWeightForging, checkDia, findShaftDia, generateChart, getBearingSpan, getWeight, tableData;
    ...
    $scope.getRievent = function(rivent, index) {
      ...
      $timeout(function() {}, rivent.shafthub === 0 ? (rivent.shafthub = rivent.old_shafthub, rivent.shaftbrg = rivent.old_shaftbrg, chartService.reportdata = rivent, chartService.inputdata = $scope.postdata) : (chartService.reportdata = rivent, chartService.inputdata = $scope.postdata), 400);
    ...
    $scope.getRow = function(data, index) {
      ...
      ReitzResources.fanseries.get({
        ...
        if ($scope.postdata.MaterialDriveControls.Width === '1') {
          ...
          return currentrow.bearingSpan = currentrow.B + currentrow.inletBoxSize1 * 0.75 + 500 + 50;
        } else {
          currentrow.A = $scope.single_double_width.A;
          currentrow.B = $scope.single_double_width.B * 1.8;
          return currentrow.bearingSpan = currentrow.B + (currentrow.inletBoxSize1 * 2) * 0.75 + 500 + 50;
        }
      });
      $timeout((function() {
        var getRpm;
        ...
        ReitzResources.shaftDiaHub.query({
          ...
          return currentrow.old_wtForging = calWeightForging(currentrow.old_shafthub, currentrow.bearingSpan);
        });
      }), 500);
    ...
    $scope.update = function(data, selectedItem, index) {
      ...
      return ReitzResources.shaftDiaHub.query({
        ...
        return currentrow.wtForging = calWeightForging(currentrow.shafthub, currentrow.bearingSpan);
      });
    };
    ...
    $scope.selectedBackPlate = function(data, index) {
      ...
      return currentrow.newGD2 = 4.2 * (currentrow.backPlate1 + currentrow.hub1 + currentrow.shroudPlate1 + currentrow.blades1) * Math.pow((checkDia($scope.row.OuterBladeDiameter1, $scope.row.OuterBladeDiameter) / 1000) * Weight_factor, 2);
    };
    ...
    tableData = function() {
      return $scope.tableParams = new ngTableParams({
        ...
        getData: (function($defer, params) {
          var fanData, fanspeed, filteredData, orderedData, rangeData;
          fanspeed = params.filter().FanSpeed;
          console.log(params.filter());
          filteredData = params.filter() ? $filter('filter')($scope.result, _.omit(params.filter(), 'FanSpeed')) : $scope.result;
          fanData = function(filteredData, fanspeed) {
            var f;
            f = fanspeed.split(',');
            if (f[1] !== void 0) {
          ...
          if (fanspeed) {
            ...
            orderedData = params.sorting() ? $filter('orderBy')(rangeData, params.orderBy()) : $scope.result;
